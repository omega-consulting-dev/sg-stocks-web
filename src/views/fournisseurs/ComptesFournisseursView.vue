...
const filteredFournisseurs = computed(() => {
  ...
  return store.fournisseurs.filter(
    (fournisseur) =>
      fournisseur.display_name?.toLowerCase().includes(query) ||
      fournisseur.first_name?.toLowerCase().includes(query) ||
      fournisseur.last_name?.toLowerCase().includes(query) ||
      fournisseur.username?.toLowerCase().includes(query) ||
      ...
      <DialogContent class="sm:max-w-[400px]">
        <DialogHeader>
          ...
          <DialogDescription v-if="fournisseurToDelete" class="pt-2">
            ...
            <strong>{{ fournisseurToDelete.display_name || fournisseurToDelete.username }}</strong> ?
            <br />
