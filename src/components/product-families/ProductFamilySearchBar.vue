...
const openSearch = () => {
  ...
  setTimeout(() => {
    const input = document.querySelector('#familySearchInput') as HTMLInputElement
    input?.focus()
  }, 100)
}
...
<template>
  <header class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
    <div>
      <h1 class="text-2xl sm:text-3xl font-bold text-primary">Famille de produits</h1>
      <p class="opacity-60 text-xs sm:text-sm">Categorisation des produits</p>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 sm:gap-4">
      ...
      <!-- Bouton Importer -->
      <DropdownMenu v-if="permissions.canManageCategories">
      ...
      <!-- Bouton Exporter -->
      <DropdownMenu v-if="permissions.canExportData || permissions.canViewCategories">
        <DropdownMenuTrigger as-child>
          <Button variant="outline" class="px-2 sm:px-4">
            <Upload />
      ...
      <!-- Bouton Nouveau -->
      <Button v-if="permissions.canManageCategories" @click="emit('add')" class="px-2 sm:px-4">
        <Plus />
