...
const openSearch = () => {
  ...
  setTimeout(() => {
    const input = document.querySelector('#serviceSearchInput') as HTMLInputElement
    input?.focus()
  }, 100)
}
...
<template>
  <header class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
    <div>
      <h1 class="text-2xl sm:text-3xl font-bold text-primary">Services</h1>
      <p class="opacity-60 text-xs sm:text-sm">Liste des services</p>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 sm:gap-4">
      ...
      <!-- Bouton Nouveau -->
      <Button v-if="permissions.canManageServices" @click="emit('add')" class="px-2 sm:px-4">
        <Plus />
