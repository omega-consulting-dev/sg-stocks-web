...
import type { Sortie } from '@/stores/sorties'
import {
  Table,
  TableBody,
  TableCell,
...
const emit = defineEmits<{
  delete: [sortie: Sortie]
  pageChange: [page: number]
}>()

// Computed
const displayedSorties = computed(() => props.sorties)
...
const paginationInfo = computed(() => {
  ...
  return `Affichage de ${start} à ${end} sur ${props.total} sorties`
})
...
<template>
  <div class="rounded-xl sm:rounded-[30px] bg-white shadow-[0px_10px_60px_0px_rgba(226,236,249,0.5)] overflow-hidden">
    <div class="overflow-x-auto">
      <Table class="min-w-[700px]">
        ...
        <TableBody>
          ...
          <template v-else-if="displayedSorties.length > 0">
            <TableRow
              v-for="sortie in displayedSorties"
              :key="sortie.id"
            ...
            >
              ...
              <TableCell class="text-[14px] font-medium text-[#292D32] text-center" style="font-family: Poppins">
                {{ sortie.movement_type === 'transfer' ? (sortie.destination_store_name || '-') : (sortie.customer_name || '-') }}
              </TableCell>
              <TableCell class="text-[14px] font-medium text-[#292D32] text-center" style="font-family: Poppins">
                {{ sortie.store_name }}
              </TableCell>
              <TableCell class="text-[14px] font-medium text-red-600 text-center" style="font-family: Poppins">
                -{{ formatQuantity(sortie.quantity) }}
              </TableCell>
              <TableCell class="text-[14px] font-medium text-[#292D32] text-center" style="font-family: Poppins">
                {{ formatDate(sortie.date || sortie.created_at) }}
              </TableCell>
    ...
    <!-- Pagination -->
    <div v-if="displayedSorties.length > 0" class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-4 sm:px-[38px] py-4 sm:py-[30px] border-t border-[#EEEEEE]">
